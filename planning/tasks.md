...
- [x] **P3-20** Publish the product demo as a native audio-enabled GitHub player and as a poster-linked browser stream on PyPI, backed by the repository-hosted MP4. Evidence: the GitHub media attachment, `docs/assets/provena-product-demo.mp4`, its poster image, README links, and release metadata validation.
- [x] **P3-21** Add one-command Claude Code and Gemini CLI onboarding with user-scoped MCP, native lifecycle hooks, protected per-host credentials, idempotent configuration merging, and cross-host local workspace reuse. Evidence: ADR 0021, `provena quickstart claude`, `provena quickstart gemini`, host installer tests, and onboarding documentation.
